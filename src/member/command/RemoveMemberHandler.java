...
public class RemoveMemberHandler implements CommandHandler{
	...
	private String processSubmit(HttpServletRequest req, HttpServletResponse res) throws IOException {
		...
		//	세션에서 user 객체를 얻기(authUser attribute)
		...
		// 세션을 invalidate() 
         ...
         return "removeMemberSuccess";
			...
			return FORM_VIEW;
		} catch (MemberNotFoundException e) { // catch 문 2개 (각 문제마다)
			res.sendError(HttpServletResponse.SC_BAD_REQUEST);
			return null;
		} 
		
			// 문제가 생기면 (문제1. 사용자가 없는 경우, 문제2. 패스워드가 틀린 경우
			// - errors 맵에 코드 추가
			// 폼.jsp로 forward 하도록  view 이름을 리턴
